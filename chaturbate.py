...
class Chaturbate(object):
	...
	def wishlist(self):
		...
		try:
			...
			f.close()
			self.log.info("wishlist.txt downloaded")
		except IOError, e:
			self.log.error("Error: %s file does not appear to exist." % "wishlist.txt")
			sys.exit(1)
		return data
	...
	def get_online_models(self):
		...
		try:
			...
			self.log.error("Something not right with models list")
		finally:
			return models
	...
	def login(self):
		...
		try:
			...
			self.log.error("Some error in login process")
			return False
	...
	def run_ffmpeg(self, model_name, source_fn, destination_fn):
		...
		arguments = [
			#["nice", "-n", "19"],
			#['ffmpeg', '-i', source_fn],
			['ffmpeg', '-nostats', '-loglevel', '0', '-y', '-threads', '4', '-i', source_fn],
